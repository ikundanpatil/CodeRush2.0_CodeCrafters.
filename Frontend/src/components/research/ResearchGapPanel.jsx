    ...
    <div className="w-full bg-[#1E293B]/90 border border-slate-700/80 rounded-[14px] p-5 shadow-xl flex flex-col gap-3">
      <div className="flex items-center justify-between pb-3 border-b border-slate-700/60">
        <div className="flex items-center gap-2">
          <AlertTriangle className="w-4 h-4 text-cyan-400" aria-hidden="true" />
          <h4 className="text-sm font-bold text-slate-100">Research Gaps</h4>
        </div>
          ...
          <ul className="space-y-1.5">
            {gaps.map((gap, idx) => (
              <li key={idx} className="flex items-start gap-2 text-xs">
                <AlertTriangle className="w-3.5 h-3.5 text-amber-400 mt-0.5 flex-shrink-0" aria-hidden="true" />
                <span className="text-slate-300 flex-1">{gap.description}</span>
